...
const Dashboard = () => {
  ...
  const fetchTotalSellingPrice = async () => {
    try {
      const response = await axios.get(
        "http://13.203.151.53:8000/dashboard/total-selling-price",
        {
          headers: {
  ...
  const downloadLogFile = async () => {
    try {
      const response = await fetch("http://13.203.151.53:8000/logs/download", {
        method: "GET",
        headers: {
  ...
  const fetchCarDetails = async () => {
    try {
      ...
      const response = await axios.get("http://13.203.151.53:8000/dashboard", {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("authToken")}`,
        },
        params,
  ...
  const fetchMonthlyCosts = async () => {
    try {
      const response = await axios.get(
        "http://13.203.151.53:8000/miscellaneous-costs/current-month",
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("authToken")}`,
          },
